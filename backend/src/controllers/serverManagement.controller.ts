...
class ServerManagementService {
  async getProjects(): Promise<ServerProject[]> {
    ...
    for (const project of projects) {
      const related = containers.filter((container) => {
        ...
        return (
          containerName.includes(project.name.toLowerCase()) ||
          (project.frontend_container &&
            containerName.includes(project.frontend_container.toLowerCase())) ||
          (project.backend_container &&
            containerName.includes(project.backend_container.toLowerCase()))
        );
      });
  ...
  async getProjectStatus(id: number) {
    ...
    const related = containers.filter((container) => {
      ...
      return name.includes(project.name.toLowerCase());
    });
    ...
    return {
      ...
      containers: related.map((container) => ({
        name: container.name,
      ...
      frontend: project.frontend_container
        ? {
            name: project.frontend_container,

            running: running.some((c) =>
              c.name.includes(project.frontend_container!),
            ),
          }
        : null,

      backend: project.backend_container
        ? {
            name: project.backend_container,

            running: running.some((c) =>
              c.name.includes(project.backend_container!),
            ),
          }
        : null,
    };
